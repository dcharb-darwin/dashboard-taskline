...
- Filter by status: dynamically populated from Admin-configurable project status list (defaults: All, Planning, Active, On Hold, Closeout, Complete)
...
| Status | Text — configurable via Admin (defaults: Planning, Active, On Hold, Closeout, Complete) | Yes |
...
| Status | Text — configurable via Admin | Yes | Defaults: Not Started, In Progress, Complete, On Hold |
| Priority | Text — configurable via Admin | Yes | Defaults: High, Medium, Low |
| Owner | String | No | |
| Start Date | Date | No | |
...
### 14.3 Statuses & Labels Tab
- **Configurable Enums**: Admin can add, remove, reorder, and color-code the available options for:
  - Project Statuses (defaults: Planning, Active, On Hold, Closeout, Complete)
  - Task Statuses (defaults: Not Started, In Progress, Complete, On Hold)
  - Task Priorities (defaults: High, Medium, Low)
  - Risk Statuses (defaults: Open, Mitigated, Accepted, Closed)
- Each option has a label and a color (blue, green, yellow, orange, red, purple, pink, gray)
- Changes are persisted to the `app_settings` table and reflected immediately across all pages (dropdowns, filter buttons, badges, calendar event colors)
- Shared type definitions in `shared/enums.ts` ensure server and client stay in sync
...
| Admin: Statuses & Labels (configurable enums) | AdminSettings | ✅ In Demo |
